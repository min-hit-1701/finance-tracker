...
public class BusinessViewModel extends AndroidViewModel {
    ...
    public void updateBusinessBudget(int id, String name, double limit, double used, String period) {
        new Thread(() -> {
            repository.updateBusinessBudgetSync(id, name, limit, used, period);
            operationMessage.postValue("Đã cập nhật ngân sách");
        }).start();
    }

    public void deleteBusinessBudget(Budget budget) {
        new Thread(() -> {
            repository.deleteBusinessBudgetSync(budget);
            operationMessage.postValue("Đã xóa ngân sách");
        }).start();
    }
    ...
    public void deleteBusinessTransaction(Transaction transaction) {
        new Thread(() -> {
            repository.deleteBusinessTransactionSync(transaction);
            operationMessage.postValue("Đã xóa giao dịch");
        }).start();
    }

    public void createBusinessPayment(String receiver, String sourceAccount, double amount, String note, long timestamp) {
        new Thread(() -> {
            AppRepository.SaveResult result = repository.createBusinessPayment(receiver, sourceAccount, amount, note, timestamp, 0, 0);
            if (result.isSuccess()) {
